...
int main() {
    ...
    if (listaVersiones.size() == 0) {
        listaVersiones.push_back(Version(1, 101, 201, "Versión A", "Acústica", 2005, "Bogotá", "Colombia", "Rock"));
        listaVersiones.push_back(Version(2, 102, 202, "Versión B", "Remix", 2010, "Medellín", "Colombia", "Pop"));
        listaVersiones.push_back(Version(3, 103, 203, "Versión C", "En vivo", 2015, "Lima", "Perú", "Jazz"));
        Version::guardarEnArchivo(nombreArchivo, listaVersiones);
    }

    cout << "Lista de versiones:\n";
    for (size_t i = 1; i <= listaVersiones.size(); i++) {
        cout << "ID: " << listaVersiones[i].getIdVersion() 
             << ", Título: " << listaVersiones[i].getTituloVer() << endl;
    }

    // Eliminar la versión (el método ya pide el ID dentro)
    Version::eliminarDeArchivo(nombreArchivo, listaVersiones);

    cout << "\nLista actualizada de versiones:\n";
    for (size_t i = 1; i <= listaVersiones.size(); i++) {
        cout << "ID: " << listaVersiones[i].getIdVersion() 
             << ", Título: " << listaVersiones[i].getTituloVer() << endl;
    }
